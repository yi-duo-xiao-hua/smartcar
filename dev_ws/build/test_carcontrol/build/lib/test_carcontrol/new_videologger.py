...
class VideoLogger(Node):
    def __init__(self):
        ...
        self.cmd_vel_sub = self.create_subscription(Twist, "/cmd_vel", self.cmd_vel_callback, 10)       # 订阅速度话题

        str = time.strftime("%m%d_%H%M%S", time.localtime())        # 获取当前时间，用于输出视频的命名
        _vd = get_code_test_root() / "output" / "videolog"
        _vd.mkdir(parents=True, exist_ok=True)
        self.video_path = str(_vd / f"video_{str}.avi")
